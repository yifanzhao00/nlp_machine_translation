...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        dec_init_state_h = torch.cat((last_hidden[0],last_hidden[1]),1)
        init_decoder_hidden = self.h_projection(dec_init_state_h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_uns = torch.unsqueeze(alpha_t,1)
        a_t = torch.bmm(alpha_t_uns,enc_hiddens)
        a_t = torch.squeeze(a_t,1)
        U_t = torch.cat((dec_hidden,a_t),1)
        V_t = self.combined_output_projection(U_t)
        tanh_V_t = torch.tanh(V_t)
        O_t = self.dropout(tanh_V_t)
